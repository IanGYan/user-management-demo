...
export const useAuth = () => {
  ...
  const verifyEmail = async (token: string) => {
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || '邮箱验证失败'
  ...
  const resendVerificationEmail = async (email?: string) => {
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || '发送验证邮件失败'
  ...
  const forgotPassword = async (email: string) => {
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || '发送重置链接失败'
